...
var LOOKUP_PATHS = module.paths.concat();
...
function getLookupPath(configFilePath) {
    ...
    if (configFilePath && pathIsInside(configFilePath, projectPath)) {
        ...
        return path.join(path.resolve(configFilePath), "node_modules");
    }
     ...
     * in this directory
     */
    return path.join(projectPath, "node_modules");
}
...
function resolve(filePath, relativeTo) {
    if (isFilePath(filePath)) {
        ...
        if (filePath.indexOf("plugin:") === 0) {
            ...
            filePath = resolver.resolve(normalizePackageName(packagePath, "eslint-plugin"), getLookupPath(relativeTo));
            return { filePath: filePath, configName: configName };
        } else {
            filePath = resolver.resolve(normalizePackageName(filePath, "eslint-config"), getLookupPath(relativeTo));
...
function load(filePath, applyEnvironments, relativeTo) {
    ...
    if (config) {
        ...
        if (config.parser) {
            config.parser = resolver.resolve(config.parser, basedir);
        }
